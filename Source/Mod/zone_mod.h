...
public:
	...
	virtual std::string GetName() const override {
		return "模组园区" + std::to_string(name);
	}

	static std::function<int(Plot*)> ZoneGenerator;

	virtual void SetZone(Plot* plot, BuildingFactory* buildingFactory) override {
...
std::function<int(Plot*)> ModZone::ZoneGenerator = [](Plot* plot) {
	return 1;
};