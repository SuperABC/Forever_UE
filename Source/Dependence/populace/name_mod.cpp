...
void NameFactory::DestroyName(NameMod* nameMod) const {
	if (!nameMod) {
		debugf("Warning: Name is null when deleting.\n");
		return;
	}

	auto it = registries.find(nameMod->GetType());
	if (it == registries.end()) {
	...
	if (it->second.second) {
		it->second.second(nameMod);
	}
	else {
		THROW_EXCEPTION(NullPointerException, "Name " + string(nameMod->GetType()) + " deleter is null.\n");
	}
}
