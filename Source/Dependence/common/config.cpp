...
std::unordered_map<std::string, std::vector<std::string>> Config::dllPaths = {};
...
void Config::RemoveDllPath(const std::string& path) {
	dllPaths.erase(path);
}
...
void Config::AddResourcePath(const std::string& path) {
...
void Config::RemoveResourcePath(const std::string& path) {
...
void Config::SetScript(const std::string& path) {
	scriptPath = path;
}
