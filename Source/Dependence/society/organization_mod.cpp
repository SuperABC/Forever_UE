...
void OrganizationFactory::DestroyOrganization(OrganizationMod* organizationMod) const {
	if (!organizationMod) {
		debugf("Warning: Organization is null when deleting.\n");
		return;
	}

	auto it = registries.find(organizationMod->GetType());
	if (it == registries.end()) {
		debugf("Warning: Organization %s not registered when deleting.\n", organizationMod->GetType());
		return;
	}

	if (it->second.second) {
		it->second.second(organizationMod);
	}
	else {
		THROW_EXCEPTION(NullPointerException, "Organization " + string(organizationMod->GetType()) + " deleter is null.\n");
	}
}
