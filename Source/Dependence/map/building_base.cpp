...
shared_ptr<Layout> Building::ReadTemplates(string path) {
...
void Building::ReadFloor(int level, int face, string name, shared_ptr<Layout>& layout) {
...
void Building::ReadFloors(int face, string name, shared_ptr<Layout>& layout) {
    for (int i = 0; i < basements + layers; i++) {
        ReadFloor(i, face, name, layout);
    }
}

void Building::ReadFloors(int face, vector<string> names, shared_ptr<Layout>& layout) {
...
void Building::AssignRoom(int level, int slot, string name, Component* component, RoomFactory* factory) {
    shared_ptr<Room> room = move(factory->CreateRoom(name));
    room->SetLayer(level);
    room->SetPosition(
...
Component* Building::CreateComponent(string name, ComponentFactory* factory) {
    shared_ptr<Component> component = move(factory->CreateComponent(name));
