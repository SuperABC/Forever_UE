		...
		for (auto job : citizen->GetJobs()) {
			...
			if (citizen->GetScheduler()->GetStatus() == "idle_home") {
				if (signin.GetYear() > 0 && time && *time > signin - Time(string("00:30:00"))) {
					auto startNode = citizen->GetHome()->GetParentBuilding()->GetParentBlock()->GetRandomRoadNode();
					auto endNode = job->GetPosition()->GetParentBuilding()->GetParentBlock()->GetRandomRoadNode();
				...
				if (signout.GetYear() > 0 && time && *time > signout) {
					auto startNode = job->GetPosition()->GetParentBuilding()->GetParentBlock()->GetRandomRoadNode();
					auto endNode = citizen->GetHome()->GetParentBuilding()->GetParentBlock()->GetRandomRoadNode();
					if (startNode && endNode) {
...
void Populace::ApplyChange(Map* map, Change* change,
	...
	if (type == "add_option") {
		...
		string genderStr = ToString(condition.EvaluateValue(getValues));
...
void Populace::GenerateCitizens(int num, Time* time) {
	...
	// 分配姓名
	name = new Name(nameFactory, nameFactory->GetName());
	if (!name) {
		THROW_EXCEPTION(RuntimeException, "No enabled name in config.\n");
	}
	for (auto story : Config::GetStories()) {
		for (auto holder : Script::ReadNames(story)) {
			name->ReserveName(holder);
		}
	}
	...
	if (sum == 0.f) {
		THROW_EXCEPTION(InvalidArgumentException, "No valid scheduler for generation.\n");
	}
	for (auto& cdf : cdfs) {
		cdf.second /= sum;
	}
	...
	for (auto citizen : citizens) {
		...
		Scheduler* scheduler = new Scheduler(schedulerFactory, selectedScheduler);
		if (!scheduler) {
